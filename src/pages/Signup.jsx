...
function Signup() {
  ...
  // Show a message if user came via invite link
  useEffect(() => {
    if (inviteId) {
      toast.info("You've been invited to join a board! Sign up to accept.", {
        autoClose: 3000,
      });
    }
  }, [inviteId]);
  ...
  const handleSignup = async (e) => {
    ...
    try {
      // Include inviteId in signup request
      ...
      if (inviteId) {
        toast.success("Signup successful! Board invitation accepted.");
      } else {
        toast.success("Signup successful!");
      }

      // Navigate to boards page after short delay
      setTimeout(() => {
        navigate("/boards");
      }, 400);
    } catch (err) {
      console.error("Signup error:", err);
      const errorMessage =
        err.data?.message || err.message || "Server error during signup";
          ...
          <div className="w-full max-w-md bg-gray-200 p-6 rounded-md shadow-md">
            ...
            <form className="space-y-4" onSubmit={handleSignup}>
              ...
              <button
                ...
                className="w-full bg-[#39393a] hover:bg-[#2c2c2d] cursor-pointer text-white font-semibold py-2 rounded-md transition duration-200"
              >
                {inviteId ? "Sign up & Join Board" : "Signup"}
              </button>
